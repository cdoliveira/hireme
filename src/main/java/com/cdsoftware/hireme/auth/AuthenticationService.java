...
import com.cdsoftware.hireme.security.JwtService;
...
public class AuthenticationService {
    ...
    public AuthenticationResponse registerWorker(RegisterRequest request) {
        var user = User.builder()
                .userName(request.getFirstname())
                .email(request.getEmail())
                .password(passwordEncoder.encode(request.getPassword()))
        ...
        var worker = Worker.builder()
                .workerName(request.getFirstname())
                .build();

        var savedWorker = workerRepository.save(worker); // CREATING THE WORKER ENTRY WHILE SAVING THE USER
        var savedUser = repository.save(user);
        var jwtToken = jwtService.generateToken(user);
        var refreshToken = jwtService.generateRefreshToken(user);
        saveUserToken(savedUser, jwtToken);
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        return AuthenticationResponse.builder()
                ...
                .userDto(UserMapper.USER_MAPPER.userToUserDto(user))
                .build();
    }
